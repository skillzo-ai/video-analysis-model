...
class PlayerReID:
    def __init__(self, device: str = "cuda" if torch.cuda.is_available() else "cpu"):
        self.device = device
        # Using a lightweight MobilenetV3 for fast embedding extraction
    ...
    @torch.no_grad()
    def extract_embedding(self, crop: np.ndarray):
        """
        Extract feature vector from a player crop (BGR).
        """
        ...
        return embedding / (np.linalg.norm(embedding) + 1e-6) # L2 normalization
